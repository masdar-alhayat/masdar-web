...
export interface FoodPortfolioItem {
  title: string;
  description: string;
...
export function FoodPortfolioCarousel({items, locale}: {items: FoodPortfolioItem[]; locale: Locale}) {
  ...
  return (
    <div
      ...
      style={{"--portfolio-visible": cardsPerView} as React.CSSProperties}
      role="region"
      aria-roledescription="carousel"
      aria-label={isRtl ? "محفظتنا الغذائية" : "Our Food Portfolio"}
      onKeyDown={(event) => {
    ...
    >
      <div className="portfolio-slider__top">
        <span className="portfolio-slider__count" aria-live="polite">
          {String(currentIndex + 1).padStart(2, "0")} / {String(maxIndex + 1).padStart(2, "0")}
        </span>
        <div className="portfolio-slider__controls">
          <button type="button" onClick={previous} disabled={currentIndex === 0} aria-label={isRtl ? "الفئات السابقة" : "Previous categories"}>
            {isRtl ? <ArrowRight aria-hidden="true"/> : <ArrowLeft aria-hidden="true"/>}
          </button>
          <button type="button" onClick={next} disabled={currentIndex === maxIndex} aria-label={isRtl ? "الفئات التالية" : "Next categories"}>
      ...
      <div
        className={`portfolio-slider__viewport${isDragging ? " is-dragging" : ""}`}
            ...
            <article key={item.title} aria-label={`${index + 1} / ${items.length}`}>
              ...
              {item.image && (
                <div className="portfolio-slider__image">
                  <Image src={item.image.src} alt="" fill sizes="(max-width: 767px) 85vw, (max-width: 1199px) 42vw, 28vw" className="portfolio-slider__image-backdrop" aria-hidden="true" draggable={false}/>
                  <Image src={item.image.src} alt={item.image.alt} fill sizes="(max-width: 767px) 85vw, (max-width: 1199px) 42vw, 28vw" className="portfolio-slider__image-foreground" draggable={false}/>
                </div>
              )}
              <h3>{item.title}</h3>
      ...
      <div className="portfolio-slider__progress" aria-hidden="true">
